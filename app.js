...
async function siapkanKatalog() {
  ...
  if (!wadah) return; // Guard clause jika bukan halaman beranda
  ...
  try {
    ...
    function perbaruiTampilan() {
      ...
      const hasil = penawaran.filter((item) => {
        const teksLengkap = `${item.nama} ${item.penyedia}`.toLowerCase();
        const cocokKata = teksLengkap.includes(kata);
  ...
  data.forEach((item) => {
    ...
    artikel.innerHTML = `
      ...
      <div class="offer-card__content">
        ...
        <h3>${item.nama}</h3>
        <p class="offer-card__meta">${item.penyedia} &bull; ${item.stok} ${item.satuan}</p>
